...
export function SearchableSelect({
  ...
  const validItems = items.filter((i) => i.value && i.value.length > 0);
  const filtered = validItems.filter((i) =>
    i.label.toLowerCase().includes(filter.toLowerCase())
  );
  const valueIsValid = validItems.some((i) => i.value === value);

  return (
    <Select.Root onValueChange={onChange} value={valueIsValid ? value : undefined}>
